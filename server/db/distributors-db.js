/**
 * Distributors table in Supabase. Scoped by org_id. data JSONB = full distributor row.
 */
...
export async function list(orgId) {
  ...
  if (error) return [];
  return (data || []).map(rowToDistributor);
}
...
export async function update(orgId, id, patch) {
  ...
  return data ? rowToDistributor(data) : null;
}

export async function remove(orgId, id) {
  const { error } = await getSupabase().from("distributors").delete().eq("id", id).eq("org_id", orgId);
  if (error) throw new Error(error.message);
  return { ok: true };
}
...
export async function deleteAllByOrgId(orgId) {
  const { error } = await getSupabase().from("distributors").delete().eq("org_id", orgId);
  if (error) throw new Error(error.message);
}
